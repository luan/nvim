...
local join_paths = require("core.util").join_paths

function M.config()
  luanvim.builtin.mason = {
    ui = {
      border = { "▄", "▄", "▄", "█", "▀", "▀", "▀", "█" },
      icons = {
        package_installed = "◍",
...
function M.get_prefix()
  ...
  return vim.tbl_get(luanvim.builtin, "mason", "install_root_dir") or default_prefix
end
...
function M.setup()
  ...
  add_to_path(luanvim.builtin.mason.PATH == "append")

  mason.setup(luanvim.builtin.mason)
end
