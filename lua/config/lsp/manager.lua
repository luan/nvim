...
local function resolve_config(server_name, ...)
  local defaults = {
    ...
    on_init = require("config.lsp").common_on_init,
    on_exit = require("config.lsp").common_on_exit,
    capabilities = require("config.lsp").common_capabilities(),
  }

  local has_custom_provider, custom_config = pcall(require, "config/lsp/providers/" .. server_name)
...
function M.setup(server_name, user_config)
  ...
  local should_auto_install = function(name)
    local installer_settings = luanvim.lsp.installer.setup
